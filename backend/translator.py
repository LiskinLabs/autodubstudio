...
class Translator:
    ...
    def translate_text(self, text, target_lang, source_lang="auto"):
        ...
        try:
            ...
            prompt = f"Translate this subtitle to {lang_name}. Output only the translation.{glossary_text}\n{text}"
            ...
            if "gemini" in self.engine_name.lower() and self.gemini_key:
                ...
                "azure" in self.engine_name.lower()
                and hasattr(self, "azure_key")
                and self.azure_key
            ):
                if OpenAI is None:
                    raise RuntimeError("openai package is not installed")
                ...
                if azure_endpoint and not _re.match(
                    r"^https://[a-zA-Z0-9_-]+\.openai\.azure\.com/", azure_endpoint
                ):
                    raise RuntimeError(
    ...
    def _call_llm(self, prompt, is_json=True):
        if "gemini" in self.engine_name.lower() and self.gemini_key:
            ...
            "openai" in self.engine_name.lower()
            and hasattr(self, "openai_key")
            and self.openai_key
        ):
            if OpenAI is None:
                raise RuntimeError("openai package is not installed")
            ...
            response_text = self.qwen_tokenizer.decode(
    ...
    def _gemma4_refine(
        ...
        total = len(segments)
        if log_callback:
            log_callback(_l("gemma4_refine_start", batch=batch_size, total=total))
...
JSON:"""
            try:
                ...
                if log_callback:
                    log_callback(
                        _l("gemma4_batch_failed", n=gemma4_failures, e=str(e)[:80])
                    )
                for seg in batch:
                    seg["text"] = seg.get("translated_base", seg["text"])

        if log_callback:
            log_callback(_l("gemma4_done"))
        self.release_models()
        return segments
...
JSON:"""
    ...
    def smart_translate_segments(
        ...
        _l = (  # noqa: E731
            lambda k, **kw: _T.get(k, {})
            .get(ui_language, _T.get(k, {}).get("en", k))
            .format(**kw)
            if kw
            else _T.get(k, {}).get(ui_language, _T.get(k, {}).get("en", k))
        )
        is_ollama = "ollama" in self.engine_name.lower()
        is_deepl = "deepl" in self.engine_name.lower() and self.deepl_key
        is_ai_refine = is_ollama or (
            ("gemini" in self.engine_name.lower() and self.gemini_key)
            or ("deepseek" in self.engine_name.lower() and self.deepseek_key)
        )
        ...
        if is_deepl:
            if log_callback:
                log_callback(_l("deepl_start"))
            import json as _json  # noqa: PLC0415
            import urllib.request  # noqa: PLC0415

            deepl_errors = 0  # noqa: F841
            ...
            for seg in segments:
                ...
                try:
                    payload = _json.dumps(
                        {"text": [orig_text], "target_lang": target_lang.upper()}
                    ).encode("utf-8")
                    headers = {
                        "Authorization": f"DeepL-Auth-Key {self.deepl_key}",
                        "Content-Type": "application/json",
        ...
        if is_ollama:
            ...
            try:
                ...
                _sp.run(
                    ["ollama", "stop", self.translator_model],
                    capture_output=True,
                    timeout=10,
                    env=os.environ.copy(),
                )
...
JSON:"""
        ...
        if is_deepl and gemma4_available:
            if log_callback:
                log_callback(_l("hybrid_refine_start"))
...
JSON:"""
                ...
                try:
                    ...
                    if log_callback:
                        log_callback(_l("smart_batch_failed", e=str(e)[:100]))
                    for seg in batch:
                        seg["text"] = seg.get("translated_base", seg["text"])
