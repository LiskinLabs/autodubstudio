...
def main():
    ...
    keys = load_keys()
    print(
        f"Keys: DeepSeek={'OK' if keys['deepseek'] else 'NO'}, "
    ...
    results = []
    for i, (lang, trans_engine, trans_model, tts_engine, demucs) in enumerate(
        TEST_CONFIGS
    ):
    ...
    report_path = os.path.join(
        OUT_DIR, f"test_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
    with open(report_path, "w", encoding="utf-8") as f:
        json.dump(
            {"test_time": datetime.now().isoformat(), "results": results},
            f,
            ensure_ascii=False,
            indent=2,
        )
    ...
    if failed:
        ...
        for r in failed:
            print(
                f"  [{r['lang'].upper()}] {r['translation']} + {r['tts']}: {r['message'][:120]}"
            )
    if passed:
        print("\nPASSED:")
        for r in passed:
            print(
                f"  [{r['lang'].upper()}] {r['translation']} + {r['tts']} ({r['duration']}s)"
            )
