...
### 🐛 Авто-репорты ошибок + краш-репорты

- Глобальный перехват ошибок UI (ErrorBoundary)
- Краш бэкенда → авто-перезапуск (Rust watchdog)
- Краш-репорты → GitHub Issues (с деталями: время, версия, платформа, стектрейс)
- **Пользователь не делает ничего** — всё автоматически

### 📥 Model Manager

- 12 локальных AI-моделей с детальным описанием (языки, размер)
- Реальный статус с бэкенда (✅ установлено / кнопка скачать)
- Прогресс-бар 0→100% для каждой модели
- Кнопка отмены загрузки с очисткой мусора
- Удаление моделей (освобождение места)
- Массовые операции: выбрать все → скачать/удалить
- Whisper (6 размеров), Demucs, Pyannote, Qwen3-TTS, XTTSv2, F5-TTS, Gemma 4
- Qwen3-TTS (~1.5 GB)
- F5-TTS (~1.2 GB)
- Gemma 4 e4b (~4.6 GB)
