    ...
    def translate_text(self, text):
        ...
        try:
            ...
            if "Gemini" in self.translator_engine and self.gemini_key:
                ...
                translated = GoogleTranslator(source="auto", target=t_lang).translate(
                    text
                )
                return f"{text}\n---\n{translated}"
        except Exception as e:
    ...
    def run(self):
        try:
            ...
            self.log_signal.emit(
                f"🚀 Loading Live Faster-Whisper ({self.model_size}) on {device}..."
            )
            model = WhisperModel(
            ...
            if (
                "Без перевода" not in self.translator_engine
                and self.target_lang == "en"
            ):
                task_mode = (
                    "translate"  # Whisper natively translates TO EN, bypassing the LLM!
                )
