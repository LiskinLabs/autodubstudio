...
for chunk in chunks:
    ...
    if not os.path.exists(
        os.path.join(out_dir, "mdx_extra_q", chunk_name, "vocals.wav")
    ):
...
with (
    open(vocals_txt, "w", encoding="utf-8") as f_v,
    open(no_vocals_txt, "w", encoding="utf-8") as f_n,
):
    for chunk in chunks:
        chunk_name = os.path.splitext(os.path.basename(chunk))[0]
        v_path = os.path.join(
            temp_dir, chunk_name, "mdx_extra_q", chunk_name, "vocals.wav"
        )
        n_path = os.path.join(
            temp_dir, chunk_name, "mdx_extra_q", chunk_name, "no_vocals.wav"
        )
...
print(
    "DONE! Vocals and no_vocals saved to target_dir. You can now run run_remaining.py."
)
